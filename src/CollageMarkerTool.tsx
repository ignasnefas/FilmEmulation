...
export default function CollageMarkerTool({ isOpen, onClose }: CollageMarkerToolProps) {
  ...
  const updatePreview = useCallback(async () => {
    ...
    if (selectedLayout.template === 'grid') {
      const cellWidth = Math.round((availableWidth - gap * (selectedLayout.cols - 1)) / selectedLayout.cols);
      const cellHeight = Math.round((availableHeight - gap * (selectedLayout.rows - 1)) / selectedLayout.rows);
      ...
      for (let i = 0; i < loadedImages.length; i++) {
        ...
        const x = Math.round(paddingPixels + col * (cellWidth + gap));
        const y = Math.round(paddingPixels + row * (cellHeight + gap));
      ...
      for (let i = 0; i < loadedImages.length && i < selectedLayout.positions.length; i++) {
        ...
        const img = loadedImages[i];
        const x = Math.round(paddingPixels + pos.col * (cellWidth + gap));
        const y = Math.round(paddingPixels + pos.row * (cellHeight + gap));
        const width = Math.round((pos.colSpan || 1) * cellWidth + (pos.colSpan ? (pos.colSpan - 1) * gap : 0));
        const height = Math.round((pos.rowSpan || 1) * cellHeight + (pos.rowSpan ? (pos.rowSpan - 1) * gap : 0));
  ...
  const getImageIndexAtPoint = (canvasX: number, canvasY: number): number | null => {
    ...
    if (selectedLayout.template === 'grid') {
      ...
      for (let i = 0; i < usedImages.length; i++) {
        const row = Math.floor(i / selectedLayout.cols);
        const col = i % selectedLayout.cols;
        const cellX = Math.round(paddingPixels + col * (cellWidth + gap));
        const cellY = Math.round(paddingPixels + row * (cellHeight + gap));
      ...
      for (let i = 0; i < usedImages.length && i < selectedLayout.positions.length; i++) {
        const pos = selectedLayout.positions[i];
        const cellX = Math.round(paddingPixels + pos.col * (cellWidth + gap));
        const cellY = Math.round(paddingPixels + pos.row * (cellHeight + gap));
        const cellW = Math.round((pos.colSpan || 1) * cellWidth + (pos.colSpan ? (pos.colSpan - 1) * gap : 0));
        const cellH = Math.round((pos.rowSpan || 1) * cellHeight + (pos.rowSpan ? (pos.rowSpan - 1) * gap : 0));
  ...
  const handleDownload = useCallback(async (width: number = 1600) => {
    ...
    if (selectedLayout.template === 'grid') {
      ...
      for (let i = 0; i < usedImages.length && i < selectedLayout.positions.length; i++) {
        ...
        const x = Math.round(paddingPixels + pos.col * (cellWidth + gap));
        const y = Math.round(paddingPixels + pos.row * (cellHeight + gap));
        const w = Math.round((pos.colSpan || 1) * cellWidth + (pos.colSpan ? (pos.colSpan - 1) * gap : 0));
        const h = Math.round((pos.rowSpan || 1) * cellHeight + (pos.rowSpan ? (pos.rowSpan - 1) * gap : 0));
        const scaleRatio = canvas.width / previewCanvasWidth;
        const scaledOffset = {
          x: (imageOffsets[i]?.x || 0) * scaleRatio,
          y: (imageOffsets[i]?.y || 0) * scaleRatio,
